...
def validate(data):
    ...
    for record in data:
        ...
        if price > 0 and category:
            ...
            reason = []
            if price <= 0:
                reason.append(f"invalid price ({price})")
            if not category:
                reason.append("empty category")
            print(f"  [DROPPED] Record {record.get('id')}: {', '.join(reason)}")

    print(f"Validation complete. Valid records: {len(valid_records)}, {error_count} dropped.")
